...
export async function providerRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Params: { provider: string }; Body: { config?: ProviderConnectionConfig; limit?: number; updatedSince?: string; importCanonical?: boolean; saveSnapshot?: boolean; markMissingInactive?: boolean; batchSize?: number; buildKnowledge?: boolean; classifyProducts?: boolean; forceClassification?: boolean; importMedia?: boolean; forceMedia?: boolean; mediaConcurrency?: number; options?: MakitoSyncOptions } }>("/providers/:provider/sync", async (request, reply) => {
    ...
    const job = jobManager.create(providerSyncPipeline, {
      ...
      input: {
        ...
        classifyProducts: request.body?.classifyProducts !== false,
        forceClassification: request.body?.forceClassification === true,
        importMedia: request.body?.importMedia !== false,
        forceMedia: request.body?.forceMedia === true,
        mediaConcurrency: request.body?.mediaConcurrency,
      },
