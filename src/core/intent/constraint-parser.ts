...
export function parseConstraints(rawText: string): ParsedConstraints {
  ...
  if (range?.[1] && range?.[2]) {
    ...
    const max = text.match(/(?:menos de|hasta|maximo|max|no pase de|por debajo de)\s*(\d+(?:[.,]\d{1,2})?)\s*(?:euros?|eur)?/);
    const min = text.match(/(?:mas de|desde|minimo|min|por encima de)\s*(\d+(?:[.,]\d{1,2})?)\s*(?:euros?|eur)?/);
    const budget = text.match(/(?:presupuesto(?: de)?|por)\s*(\d+(?:[.,]\d{1,2})?)\s*(?:euros?|eur)/);
  ...
  const asksPersonalization = /(?:personaliz|grabad|con nombre|con foto|con logo|a medida)/.test(text);
