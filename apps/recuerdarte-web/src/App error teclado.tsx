...
function textFromResponse(value: Record<string, unknown>): string {
  const candidates = [value.reply, value.nextQuestion, value.message];
  return candidates.find((item): item is string => typeof item === "string") ??
    "Ya siento un poco mejor lo que quieres crear. Sigamos dando forma a este recuerdo.";
}
...
export function App() {
  ...
  const client = useMemo(() => new ExperienceSdkClient({ baseUrl: "/api/v1", credentials: saved.credentials }), [saved.credentials]);
  ...
  const [storyExperienceOpen, setStoryExperienceOpen] = useState(false);
  ...
  const [imageExperienceOpen, setImageExperienceOpen] = useState(false);
  ...
  const [imageProgressMessage, setImageProgressMessage] = useState<string>();
  ...
  const imageBriefs = useMemo(() => imageBriefArtifacts.flatMap((item) => {
  ...
  const stories = useMemo(() => storyArtifacts.flatMap((item) => {
  ...
  const activeStage = useMemo<StageKey>(() => {
    ...
    if (mockups.length > 0 || recommendations.length > 0) return "gift";
    if (images.length > 0) return "image";
    if (stories.length > 0) return "story";
    return "conversation";
  }, [images.length, mockups.length, orders, recommendations.length, stories.length]);
  ...
  async function submit(event?: FormEvent): Promise<void> {
    ...
    try {
      ...
      const nextJourneyId = response.journey?.id ?? journeyId;
      if (nextJourneyId) {
  ...
  async function generateImage(): Promise<void> {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 120; attempt += 1) {
        ...
        const task = await client.getTask<Record<string, unknown>>(accepted.taskId);
        ...
        if (state === "COMPLETED") {
          ...
          setTimeout(() => setGeneratingImage(false), motionRuntime.completedStateHold);
          return;
        }
        if (["FAILED", "CANCELLED"].includes(state)) throw new Error(String(task.error ?? "La generación no pudo completarse."));
      }
      throw new Error("La imagen está tardando más de lo esperado. Puedes volver a abrir la galería dentro de un momento.");
    } catch (reason) {
  ...
  if (!hasBegun) {
    return <main id="main-content" className="landingShell">
  ...
  return <main id="main-content" className="appShell">
    ...
    <section className="experienceLayout">
      ...
      <aside className="memoryCanvas">
        ...
        {!experience ? <div className="emptyCanvas">
